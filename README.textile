...
<pre><code>
  bin/cassandra-bulkload --host=<ip_of_some_cassandra_seed> --loadtable --ks=MyKeySpace --cf=MyColumnFamily --col_names=user_id,screen_name,ip_address /path/to/tsv-table
</code></pre>
...
<pre><code>
  bin/cassandra-bulkload --host=<ip_of_some_cassandra_seed> --loadcolumns --ks=MyGraphKeyspace --cf=MyGraphColumnFamily /path/to/adj-list
</code></pre>
...
<pre><code>
  bin/cassandra-bulkload --host=<ip_of_some_cassandra_seed> --loadmap --ks=MyKeyspace --cf=MyColumnFamily /path/to/adj-list
</code></pre>

If your data isn't already in this form there is wukong script in the examples directory that takes input of the form (row_key,column_name,column_value) and produces output necessary for the map loader. 
