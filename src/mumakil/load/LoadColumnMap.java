        ...
        public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            ...
            if(longNames == 1) {
                for(Map.Entry pair: columns.entrySet()) {
                    try {
                        rowMutationList.add(CassandraUtils.getMutation(CassandraUtils.stringToLongBytes(pair.getKey().toString()), pair.getValue().toString().getBytes(), System.currentTimeMillis() * 1000));
                ...
                for(Map.Entry pair: columns.entrySet()) {
                    rowMutationList.add(CassandraUtils.getMutation(pair.getKey().toString().getBytes(), pair.getValue().toString().getBytes(), System.currentTimeMillis() * 1000));
                    context.write(rowKey, rowMutationList);
                    rowMutationList.clear();
                }
            }
